...
public class ExpenseManagementControllerTests
{
    private readonly Mock<IExpenseService> _mockExpenseService;
    private readonly Mock<ILogger<ExpenseManagementController>> _mockLogger;
    private readonly ExpenseManagementController _controller;

    public ExpenseManagementControllerTests()
    {
        _mockExpenseService = new Mock<IExpenseService>();
        _mockLogger = new Mock<ILogger<ExpenseManagementController>>();
        _controller = new ExpenseManagementController(_mockExpenseService.Object, _mockLogger.Object);
    }
    ...
    public async Task UpdateExpense_ReturnsOkResult_WhenExpenseExists()
    {
        ...
        // Act
        var result = await _controller.UpdateExpense(expenseId, expenseRequest);
    ...
    [Fact]
    public async Task UpdateExpense_ReturnsBadRequest_WhenArgumentExceptionThrown()
    {
        // Arrange
        var expenseId = 1;
        var expenseRequest = new ExpenseRequestDto
